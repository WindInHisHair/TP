...
// CUT begin
ifstream data("WidgetRepairs.sample");
...
bool do_test(vector<int> arrivals, int numPerDay, int __expected) {
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(arrivals, numPerDay, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1450362912;
